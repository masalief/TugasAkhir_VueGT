...
public class userRepo implements IuserRepo {
	...
	public usermodel insertUser(usermodel usermodel) {
		...
		String query = "INSERT INTO tb_user(" 
			...
			+ "password )"
			+ "VALUES(?, ?, ?)";
		jdbcTemplate.update(query, new Object[] {usermodel.getEmail(),
				usermodel.getNama(),
	...
	public usermodel insertusermodel(usermodel usermodel) {
		...
		String query = "INSERT INTO tb_user(email, nama, password)" 
				+ "VALUES(?,?,?)";
		jdbcTemplate.update(query, new Object[] {usermodel.getEmail(), usermodel.getNama(), usermodel.getPassword(),});
		return null;
	}
